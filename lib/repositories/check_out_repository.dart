...
// chekout of mart
Future<CheckoutModelMart> checkoutRepoOfMart(
) async {
  SharedPreferences pref = await SharedPreferences.getInstance();
  LoginModel? user =
  ...
  try {
    final response =
    await http.get(Uri.parse("${ApiUrl.checkoutOfmart}"), headers: headers);
    if (response.statusCode == 200) {
      log("Chekcout data of mart...${response.body}");
      return CheckoutModelMart.fromJson(jsonDecode(response.body));
    } else {
      throw Exception(response.body);
    }
  } catch (e) {
